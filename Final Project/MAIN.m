...
for i=1:Mpc
    if i<Mpc-N
        p_sampled= p_s(i:i+N);
        [feas(i), vOpt_ol, uOpt_ol] = train_batch(N,v0,p_sampled,dp,vgrid,Fgrid,E_grid,profile);
        vOpt(i+1)=vOpt_ol(2);
        uOpt(i)=uOpt_ol(1);
        v0=vOpt(i+1);
...
figure(1)
subplot(3,1,1)
plot(p_s(1:Mpc-3),vOpt(1:Mpc-3))
hold on;
subplot(3,1,2)
plot(p_s(1:Mpc-3),uOpt(1:Mpc-3))
hold on;
subplot(3,1,3)
plot(p_s(1:Mpc-3),sl(1:Mpc-3))
hold on;

%%
figure(2)
contourf(vgrid,Fgrid,E_grid)
